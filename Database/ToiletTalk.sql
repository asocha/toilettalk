...
use ToiletTalk;
create table users(user_id INT AUTO_INCREMENT, username VARCHAR(16), first_name VARCHAR(16), last_name VARCHAR(16), password VARCHAR(120), gender TINYINT(1), permission INT, PRIMARY KEY(user_id));
...
create table routes(route_id INT AUTO_INCREMENT, user_id INT, origin VARCHAR(200), destination VARCHAR(200), PRIMARY KEY(route_id), FOREIGN KEY (user_id) REFERENCES users(user_id));
